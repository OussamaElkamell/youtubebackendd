...
function selectNextAccount(accounts, lastUsedAccountId) {
  ...
  const lastUsed = lastUsedAccountId || null;
  console.log('lastUsed:', lastUsed);
  console.log("accounts:", accounts.map(a => a.id));

  // Find next account that wasn't the last used one
  let nextAccount = accounts.find(acc => acc.id !== lastUsed);
  ...
  console.log('Selected account:', nextAccount?.id);
  return nextAccount;
}
